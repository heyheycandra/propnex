...
class OnAirRow extends StatelessWidget {
  const OnAirRow({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => SerialBloc()..add(GetSerialNP()),
      child: const OnAirRowBody(),
    );
  }
}

class OnAirRowBody extends StatefulWidget {
  const OnAirRowBody({Key? key}) : super(key: key);

  @override
  State<OnAirRowBody> createState() => _OnAirRowBodyState();
}

class _OnAirRowBodyState extends State<OnAirRowBody> {
                      ...
                      ? List.generate(
                          ...
                          (index) {
                            if (index == listfilms.length - 1 || index == 4) {
                              return SeeMoreRow(
                                onTap: () {
                                  locator<NavigatorService>().navigateTo(Constant.menuSeriesNP);
                                },
                              );
                            } else {
